...
public partial struct PlayerHealthSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        {
            return; // hit invincibility, nothing to do
        }

        var playerPosition = SystemAPI.GetComponentRO<LocalTransform>(playerHealth.ValueRO.Entity).ValueRO.Position;

        foreach (var enemyTransform in SystemAPI.Query<RefRO<LocalTransform>>().WithAll<Enemy>())
        {
            if(math.distancesq(playerPosition,enemyTransform.ValueRO.Position) > 0.1f)
            {
                continue;
            }
